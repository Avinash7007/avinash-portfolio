...
(function () {
  ...
  (function () {
    ...
    var body = 'Hi Avinash,\n\nI came across your portfolio and would like to connect regarding a Data Analyst / BI opportunity.\n\n';
    var mailtoHref = 'mai' + 'lto:' + CONTACT_EMAIL +
      '?subject=' + encodeURIComponent(subject) +
      '&body=' + encodeURIComponent(body);
    if (ci) ci.href = mailtoHref;
    if (btn) btn.href = mailtoHref;
  })();
    ...
    a.addEventListener('click', function (e) {
      if (!IS_TOUCH_DEVICE) {
        ...
        var gmailUrl = 'https://mail.google.com/mail/?view=cm&fs=1&to=' +
          ...
          '&body=' + encodeURIComponent('Hi Avinash,\n\nI came across your portfolio and would like to connect regarding a Data Analyst / BI opportunity.\n\n');
  ...
  (function () {
    ...
    var sectionIds = ['about', 'skills', 'projects', 'experience', 'education', 'certificates', 'contact'];
    var seen = {};
    ...
    // Like bind(), but also attaches a param computed from the clicked
    // element (e.g. which project or which certificate) — same event
    // name, richer detail in GA4 reports.
    function bindWithParam(selector, eventName, getParams) {
      document.querySelectorAll(selector).forEach(function (el) {
        el.addEventListener('click', function () { track(eventName, getParams(el)); });
      });
    }
    ...
    bind('a[href$=".pdf"][download]', 'resume_download_click');

    // Certificate links — same 'certificate_click' event as before, now
    // with which certificate was clicked so reports show per-cert interest.
    bindWithParam('.cert-link', 'certificate_click', function (el) {
      var card = el.closest('.cert');
      var nameEl = card ? card.querySelector('.cert-name') : null;
      return { certificate_name: nameEl ? nameEl.textContent.trim() : '' };
    });

    // Project "View on GitHub" links — fires alongside the generic
    // github_click above, but names which project was viewed.
    bindWithParam('.proj-link', 'project_view_click', function (el) {
      var card = el.closest('.proj');
      var nameEl = card ? card.querySelector('.proj-name') : null;
      return { project_name: nameEl ? nameEl.textContent.trim() : '' };
    });
